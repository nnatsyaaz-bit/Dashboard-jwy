        ...
        <div class="col-md-4">
            <div class="card card-primary card-outline">
                <div class="card-body box-profile text-center">
                    ...
                    <h3 class="profile-username text-center font-weight-bold">{{ $profil->nama ?? 'Nama Mahasiswa' }}</h3>
                    <p class="text-muted text-center"><i class="fas fa-graduation-cap text-primary mr-1"></i> {{ $profil->prodi ?? 'Teknologi Rekayasa Perangkat Lunak' }}</p>

                    <ul class="list-group list-group-unbordered mb-3 text-left">
                        <li class="list-group-item">
                            <b><i class="fas fa-id-card mr-2 text-muted"></i>NIM</b> <a class="float-right text-dark">{{ $profil->nim ?? '-' }}</a>
                        </li>
                        <li class="list-group-item">
                            <b><i class="fas fa-venus-mars mr-2 text-muted"></i>Gender</b> <a class="float-right text-dark">{{ ucfirst($profil->jenis_kelamin ?? '-') }}</a>
                        </li>
                        <li class="list-group-item">
                            <b><i class="fas fa-phone mr-2 text-muted"></i>Telepon</b> <a class="float-right text-dark">{{ $profil->telp ?? '-' }}</a>
                        </li>
                        <li class="list-group-item">
                            <b><i class="fas fa-envelope mr-2 text-muted"></i>Email</b> <a class="float-right text-dark">{{ $profil->email ?? '-' }}</a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6 form-group">
                                        ...
                                        <input type="text" name="prodi" class="form-control" value="{{ $profil->prodi ?? 'Teknologi Rekayasa Perangkat Lunak' }}">
                                    </div>
                                    <div class="col-md-6 form-group">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 form-group">
                                        ...
                                        <input type="text" name="telp" class="form-control" value="{{ $profil->telp ?? '' }}">
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label>Hobi / Minat</label>
                                    <input type="text" name="hobi" class="form-control" value="{{ $profil->hobi ?? '' }}">
                                </div>
